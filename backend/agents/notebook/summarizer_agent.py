...
from core.llm.prompt_templates import SUMMARIZER, render
...
class SummarizerAgent(BaseAgent):
    ...
    async def _summarise_once(
        ...
        *,
        ...
        system_prompt = render(SUMMARIZER, max_bullets=max_bullets)
        user_prompt = _build_user_prompt(
            "\n\n".join(texts), style=style, max_bullets=max_bullets
        )
    ...
    async def _summarise_map_reduce(
        ...
        *,
        ...
        partials: list[str] = []
        map_system_prompt = render(SUMMARIZER, max_bullets=per_window_bullets)
        for window_text in windows:
            user_prompt = _build_user_prompt(
                window_text, style="bullets", max_bullets=per_window_bullets
            )
        ...
        reduce_system_prompt = render(SUMMARIZER, max_bullets=max_bullets)
        combined = "\n\n".join(
            f"--- partial {i + 1} ---\n{p}" for i, p in enumerate(partials)
        )
        reduce_user_prompt = _build_user_prompt(
            combined, style=style, max_bullets=max_bullets
        )
        return await self.llm.complete(
            [
...
def _build_user_prompt(source_text: str, *, style: str, max_bullets: int) -> str:
    """Wrap source text with style instruction."""
